defmodule Dunda.ScraperTest do
      ...
      Normaliser.normalise(
        [
          %{
            ...
            "title" => "Test Event",
            "venue" => "Test Venue",
            "date_text" => "2026-06-10T14:00:00Z",
            "price_text" => "KES 1,200",
            "url" => "https://example.com/events/ext-123"
          }
        ],
        :html,
        organisation_id: 42
  ...
  test "provider rows with invalid timestamps are rejected rather than partially persisted" do
    assert Normaliser.normalise(
             [%{"id" => "fb-1", "name" => "Broken", "start_time" => "not-a-date"}],
             :facebook
           ) == []
  end
  ...
  test "generic HTML JSON-LD fixture produces a canonical event" do
    html =
      ~s(<script type="application/ld+json">{"@type":"Event","@id":"jsonld-1","name":"JSON-LD Night","startDate":"2026-06-10T14:00:00Z","location":{"name":"Venue"}}</script>)
