defmodule Dunda.Ticketing.CredentialProtocolTest do
  ...
  @vector %{
    ...
    credential_public_key: Base.url_decode64!("11qYAYKxCrfVS_7TyWQHOg7hcvPapiMlrwIaaPcHURo", padding: false),
    signature: Base.url_decode64!("3bG11yTivfz4rdoDYaB2H9ULh4r-vbjRjC9FBC7Dl6M76S1q3k2uMIUrItYDSpbZFSHMbmKFqNlGFm9mW_ykDA", padding: false)
  }

  test "canonical proof matches the published cross-language vector" do
    assert CredentialProtocol.canonical_proof(@vector) == "dunda-ticket-proof\nv=2\nticket_id=ticket-001\nevent_id=42\ntime_step=57600000\nnonce=ABEiM0RVZneImaq7zN3u_w\ncredential_public_key=11qYAYKxCrfVS_7TyWQHOg7hcvPapiMlrwIaaPcHURo"
    assert CredentialProtocol.verify_device_signature(@vector.credential_public_key, CredentialProtocol.canonical_proof(@vector), @vector.signature)
  end
