  ...
  def update_request(conn, %{"id" => id} = params) do
    ...
    case Repo.get_by(DataSubjectRequest, id: id, user_id: user.id) do
      ...
      %{request_type: "rectification"} ->
        respond_dsr_update(conn, Privacy.process_rectification(user.id, id, Map.take(params, ["name"])))

      %{request_type: "objection"} ->
        respond_dsr_update(conn, Privacy.record_objection(user.id, id, params["scope"]))

      _other ->
        conn |> put_status(:unprocessable_entity) |> json(%{error: %{code: "request_type_not_updatable"}})
    end
  end

  defp respond_dsr_update(conn, {:ok, request}) do
    json(conn, %{data: %{id: request.id, request_type: request.request_type, status: request.status}})
  end

  defp respond_dsr_update(conn, {:error, :not_found}), do: conn |> put_status(:not_found) |> json(%{error: %{code: "request_not_found"}})
  defp respond_dsr_update(conn, {:error, :wrong_request_type}), do: conn |> put_status(:unprocessable_entity) |> json(%{error: %{code: "request_type_not_updatable"}})
  defp respond_dsr_update(conn, {:error, :invalid_transition}), do: conn |> put_status(:unprocessable_entity) |> json(%{error: %{code: "invalid_transition"}})
