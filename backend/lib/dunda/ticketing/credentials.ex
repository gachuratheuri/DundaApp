defmodule Dunda.Ticketing.Credentials do
  ...
  def device_challenge(ticket_id, user_id) do
    if Dunda.Containment.blocked?(:ticket_credentials),
      do: {:error, :phase_0_containment},
      else: device_challenge_open(ticket_id, user_id)
  end
  ...
  defp device_challenge_open(ticket_id, user_id) do
    with %Ticket{} = ticket <- Repo.get_by(Ticket, id: ticket_id, user_id: user_id),
      ...
      token =
        Phoenix.Token.sign(
          DundaWeb.Endpoint,
          "ticket-device-binding",
          %{ticket_id: ticket.id, user_id: user_id, challenge: challenge},
  ...
  def bind_device(ticket_id, user_id, token, public_key, signature, step_up_token \\ nil) do
    if Dunda.Containment.blocked?(:ticket_credentials),
      do: {:error, :phase_0_containment},
      else: bind_device_open(ticket_id, user_id, token, public_key, signature, step_up_token)
  end

  defp bind_device_open(ticket_id, user_id, token, public_key, signature, step_up_token) do
    with {:ok, claims} <-
           Phoenix.Token.verify(DundaWeb.Endpoint, "ticket-device-binding", token,
             max_age: @challenge_max_age
           ),
         true <-
           to_string(Map.get(claims, :ticket_id) || Map.get(claims, "ticket_id")) ==
             to_string(ticket_id) and
             to_string(Map.get(claims, :user_id) || Map.get(claims, "user_id")) ==
               to_string(user_id),
         ...
         true <-
           CredentialProtocol.verify_device_signature(
             public_key,
             CredentialProtocol.canonical_binding(ticket_id, user_id, challenge),
             signature
           ) do
  ...
  defp bind_locked(ticket_id, user_id, public_key, step_up_token) do
    Repo.transaction(fn ->
      ...
      jwt =
        Entitlement.mint_device_bound(ticket.id, public_key,
          valid_from: valid_from,
          valid_until: valid_until,
          event_id: event.id,
          claims: [{"credential_epoch", epoch}]
        )
      ...
      _ =
        Dunda.Audit.record(%{
          action: "ticket.credential_#{event_type}",
          resource_type: "ticket",
          resource_id: ticket.id,
          actor_id: user_id,
          metadata: %{credential_epoch: epoch, protocol_version: 2}
        })
    ...
    Repo.transaction(fn ->
      ...
      Repo.insert!(
        TicketCredentialEvent.changeset(%TicketCredentialEvent{}, %{
          ...
          metadata: %{reason: String.slice(to_string(reason), 0, 240)},
          occurred_at: now
        })
      )
  ...
  defp fingerprint(public_key),
    do: Base.url_encode64(:crypto.hash(:sha256, public_key), padding: false)
end
