defmodule Dunda.Ticketing.Ticket do
  ...
  schema "tickets" do
    ...
    belongs_to :ticket_batch, Dunda.Checkout.TicketBatch, type: :binary_id
  ...
  defp validate_credential_consistency(changeset) do
    ...
    cond do
      version not in [1, 2] ->
        add_error(changeset, :credential_version, "must be protocol version 1 or 2")

      version == 2 and (not is_binary(public_key) or byte_size(public_key) != 32) ->
        add_error(changeset, :credential_public_key, "must be a 32-byte Ed25519 public key")

      version == 2 and (is_nil(from) or is_nil(until) or DateTime.compare(until, from) != :gt) ->
        add_error(changeset, :credential_valid_until, "must be after credential_valid_from")

      version == 2 and is_nil(bound_at) ->
        add_error(changeset, :credential_bound_at, "is required for device-bound credentials")
