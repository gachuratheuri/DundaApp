defmodule Dunda.Ticketing.ScannerDevices do
  ...
  def register(operator_id, attrs) do
    if Dunda.Containment.blocked?(:scanner_admission),
      ...
      else: register_open(operator_id, attrs)
  end

  defp register_open(operator_id, attrs) do
    with {:ok, public_key} <-
           CredentialProtocol.decode(attrs[:device_public_key] || attrs["device_public_key"]),
         true <- CredentialProtocol.valid_public_key?(public_key),
         {:ok, event_id} <- event_id(attrs[:event_id] || attrs["event_id"]),
         %Dunda.Events.Event{organisation_id: organisation_id} <-
           Repo.get(Dunda.Events.Event, event_id),
         :ok <-
           require_membership(organisation_id, operator_id, [
             "owner",
             "admin",
             "manager",
             "scanner"
  ...
  def revoke(device_id, actor_id, reason) do
    if Dunda.Containment.blocked?(:scanner_admission),
      do: {:error, :phase_0_containment},
      else: revoke_open(device_id, actor_id, reason)
  end
  ...
  defp revoke_open(device_id, actor_id, reason) do
    ...
    with %ScannerDevice{} = device <- device,
         :ok <-
           require_membership(device.organisation_id, actor_id, ["owner", "admin", "manager"]) do
      now = DateTime.utc_now() |> DateTime.truncate(:second)

      Repo.update(
        ScannerDevice.changeset(device, %{
          status: "revoked",
          revoked_at: now,
          revocation_reason: String.slice(to_string(reason), 0, 240)
        })
      )
    else
  ...
  defp require_membership(organisation_id, user_id, roles) do
    if Repo.exists?(
         from m in Dunda.Organisations.OrganisationMember,
           where:
             m.organisation_id == ^organisation_id and m.user_id == ^user_id and m.role in ^roles and
               not is_nil(m.accepted_at)
       ),
       do: :ok,
  ...
  defp event_id(value) when is_binary(value) do
