defmodule Dunda.Organisations.PayoutBatch do
  ...
  def changeset(batch, attrs) do
    batch
    |> cast(attrs, [
      :amount_cents, :currency, :status, :idempotency_key, :b2c_conversation_id,
      :b2c_receipt, :failure_reason, :period_start, :period_end, :submitted_at,
      :paid_at, :submission_started_at, :organisation_id
    ])
  ...
  defp validate_transition(changeset) do
    case get_change(changeset, :status) do
      ...
      next ->
        ...
        if next in Map.get(allowed, current, []), do: changeset, else: add_error(changeset, :status, "invalid monotonic payout transition")
    end
  end
end
