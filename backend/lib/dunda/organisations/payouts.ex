defmodule Dunda.Organisations.Payouts do
  ...
  def reconcile_provider_result(conversation_id, result_code, receipt) do
    Repo.transaction(fn ->
      ...
      attrs =
        if success? do
          %{status: "paid", b2c_receipt: receipt, paid_at: DateTime.utc_now() |> DateTime.truncate(:second)}
        else
    ...
    Repo.transaction(fn ->
      case Repo.one(
             from b in PayoutBatch,
               where: b.organisation_id == ^organisation_id and b.status in ["pending", "submitting", "submitted"],
        ...
        nil ->
              ...
              from o in Order,
                where:
                  o.organisation_id == ^organisation_id and o.kind == "primary" and
                    o.status in ["completed", "partially_refunded"] and o.payout_status == "unpaid",
                order_by: [asc: o.id],
          ...
          if orders == [] do
            ...
            key = "payout:#{organisation_id}:" <> Base.url_encode64(:crypto.hash(:sha256, Enum.map(orders, &to_string(&1.id)) |> Enum.join(",")), padding: false)

            with {:ok, batch} <-
                   %PayoutBatch{}
                   |> PayoutBatch.changeset(%{organisation_id: organisation_id, amount_cents: amount, currency: "KES", idempotency_key: key, period_start: period_start, period_end: period_end})
                   |> Repo.insert(),
              ...
              _ = Dunda.Audit.record(%{action: "payout.batch_created", resource_type: "payout_batch", resource_id: batch.id, metadata: %{organisation_id: organisation_id, amount_cents: amount, item_count: length(orders)}})
              batch
            else
              {:error, changeset} -> Repo.rollback(changeset)
              {:error, reason} -> Repo.rollback(reason)
  ...
  def claim_batch_submission(%PayoutBatch{id: id}) do
    Repo.transaction(fn ->
      ...
      cond do
        is_nil(batch) -> Repo.rollback(:payout_batch_not_found)
        batch.status == "pending" ->
          case batch |> PayoutBatch.changeset(%{status: "submitting", submission_started_at: now()}) |> Repo.update() do
            {:ok, claimed} -> claimed
            {:error, changeset} -> Repo.rollback(changeset)
          end
        batch.status == "submitting" and stale_submission?(batch) ->
          case batch |> PayoutBatch.changeset(%{status: "manual_review", failure_reason: "submission claim exceeded #{@submission_timeout_seconds}s; verify provider before retry"}) |> Repo.update() do
            {:ok, reviewed} -> reviewed
            {:error, changeset} -> Repo.rollback(changeset)
          end
        batch.status in ["submitting", "submitted"] -> batch
        true -> Repo.rollback({:payout_batch_not_claimable, batch.status})
      end
    end)
    |> case do
      {:ok, batch} -> {:ok, batch}
  ...
  @spec mark_batch_submitted(PayoutBatch.t(), String.t()) :: {:ok, PayoutBatch.t()} | {:error, term()}
  def mark_batch_submitted(%PayoutBatch{status: status} = batch, conversation_id)
    ...
    batch
    |> PayoutBatch.changeset(%{status: "submitted", b2c_conversation_id: conversation_id, submitted_at: DateTime.utc_now() |> DateTime.truncate(:second)})
    |> Repo.update()
  end

  def mark_batch_submitted(%PayoutBatch{} = batch, _), do: {:error, {:invalid_payout_transition, batch.status}}

  @spec mark_batch_manual_review(PayoutBatch.t(), term()) :: {:ok, PayoutBatch.t()} | {:error, term()}
  def mark_batch_manual_review(%PayoutBatch{} = batch, reason) do
    case batch |> PayoutBatch.changeset(%{status: "manual_review", failure_reason: inspect(reason)}) |> Repo.update() do
      {:ok, updated} = result ->
        _ = Dunda.Audit.record(%{action: "payout.batch_manual_review", resource_type: "payout_batch", resource_id: batch.id, metadata: %{reason: inspect(reason)}})
        result
  ...
  def reconcile_batch_provider_result(conversation_id, result_code, receipt) do
    Repo.transaction(fn ->
      batch = Repo.one(from b in PayoutBatch, where: b.b2c_conversation_id == ^conversation_id, lock: "FOR UPDATE")

      cond do
        is_nil(batch) -> Repo.rollback(:payout_batch_not_found)
        batch.status == "paid" -> batch
        batch.status == "failed" -> batch
        to_string(result_code) == "0" ->
          now = DateTime.utc_now() |> DateTime.truncate(:second)
          item_order_ids = Repo.all(from i in PayoutItem, where: i.payout_batch_id == ^batch.id, select: i.order_id)
          {_, _} = from(i in PayoutItem, where: i.payout_batch_id == ^batch.id, update: [set: [status: "paid"]]) |> Repo.update_all([])
          {_, _} = from(o in Order, where: o.id in ^item_order_ids, update: [set: [payout_status: "paid", updated_at: ^now]]) |> Repo.update_all([])

          case batch |> PayoutBatch.changeset(%{status: "paid", b2c_receipt: receipt, paid_at: now}) |> Repo.update() do
            {:ok, paid} ->
              _ = Dunda.Audit.record(%{action: "payout.batch_paid", resource_type: "payout_batch", resource_id: batch.id, metadata: %{result_code: result_code}})
              paid
            {:error, changeset} -> Repo.rollback(changeset)
          end
        ...
        true ->
          ...
          now = DateTime.utc_now() |> DateTime.truncate(:second)
          item_order_ids = Repo.all(from i in PayoutItem, where: i.payout_batch_id == ^batch.id, select: i.order_id)
          {_, _} = from(i in PayoutItem, where: i.payout_batch_id == ^batch.id, update: [set: [status: "failed", failure_reason: ^failure_reason]]) |> Repo.update_all([])
          {_, _} = from(o in Order, where: o.id in ^item_order_ids, update: [set: [payout_status: "unpaid", updated_at: ^now]]) |> Repo.update_all([])

          case batch |> PayoutBatch.changeset(%{status: "failed", failure_reason: failure_reason}) |> Repo.update() do
            {:ok, failed} ->
              _ = Dunda.Audit.record(%{action: "payout.batch_failed", resource_type: "payout_batch", resource_id: batch.id, metadata: %{result_code: result_code}})
              failed
            {:error, changeset} -> Repo.rollback(changeset)
          end
      end
  ...
  defp insert_items(batch, orders) do
    Enum.reduce_while(orders, {:ok, []}, fn order, {:ok, acc} ->
      case %PayoutItem{} |> PayoutItem.changeset(%{payout_batch_id: batch.id, order_id: order.id, amount_cents: net_amount(order), currency: order.currency}) |> Repo.insert() do
