defmodule Dunda.Payments do
  ...
  def register_checkout_id(checkout_request_id) do
    Registry.register(@registry, {:cri, checkout_request_id}, nil)
    :ok
  end
  ...
  def deliver_callback(checkout_request_id, stk_callback) do
    case Registry.lookup(@registry, {:cri, checkout_request_id}) do
      [{pid, _}] ->
  ...
  defp via(transaction_id), do: {:via, Registry, {@registry, transaction_id}}
