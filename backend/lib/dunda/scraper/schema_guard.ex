defmodule Dunda.Scraper.SchemaGuard do
  ...
  def api_events(source, %{"events" => events}) when source == "eventbrite" and is_list(events),
    do: {:ok, events}

  def api_events(source, %{"data" => data})
      when source in ["facebook", "instagram"] and is_list(data),
      do: {:ok, data}
  ...
  def report_empty(source, target) do
    ...
    Logger.warning(
      "scraper source returned zero events source=#{source} target=#{Dunda.Security.URL.log_safe(target)}"
    )
