defmodule Dunda.Workers.FinancialReconciliationWorker do
  ...
  def perform(%Oban.Job{}) do
    if Dunda.Containment.blocked?(:checkout) do
      ...
      intents = Repo.all(from p in PaymentIntent, where: p.state in ["confirmed", "fulfilled", "confirmed_late"], limit: 1_000)

      diff_count =
        Enum.count(intents, fn intent ->
          if is_nil(Repo.get_by(JournalTransaction, reference: "settlement:#{intent.id}")) do
            _ = Dunda.Checkout.advance_state(intent, "manual_review", %{reason: "settlement_journal_missing"})
            true
          else
            false
          end
