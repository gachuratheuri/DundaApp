defmodule Dunda.Checkout.QuoteSigner do
  ...
  def valid?(attrs, signature) when is_binary(signature) do
    expected = sign(attrs)
    byte_size(expected) == byte_size(signature) and Plug.Crypto.secure_compare(expected, signature)
  end
  def valid?(_, _), do: false

  def canonical(attrs) do
    [attrs.user_id, attrs.event_id, attrs.ticket_tier_id || "event", attrs.quantity,
     attrs.unit_price_cents, attrs.fee_cents, attrs.total_cents, attrs.currency,
     attrs.price_version, DateTime.to_iso8601(attrs.expires_at)]
