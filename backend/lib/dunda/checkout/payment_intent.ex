defmodule Dunda.Checkout.PaymentIntent do
  ...
  def changeset(intent, attrs) do
    intent
    |> cast(attrs, [:quote_id, :user_id, :event_id, :ticket_tier_id, :quantity, :amount_cents, :currency, :phone_encrypted, :idempotency_key, :state, :provider, :provider_checkout_id, :redirect_url, :provider_receipt, :failure_reason, :manual_review_reason, :expires_at, :confirmed_at, :fulfilled_at, :version])
    |> validate_required([:quote_id, :user_id, :event_id, :quantity, :amount_cents, :currency, :phone_encrypted, :idempotency_key, :state, :expires_at, :version])
  ...
  defp validate_transition(changeset) do
    case get_change(changeset, :state) do
      ...
      next ->
        ...
        if transition_allowed?(current, next), do: changeset, else: add_error(changeset, :state, "invalid monotonic payment transition")
    end
  end
end
